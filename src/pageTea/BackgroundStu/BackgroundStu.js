...
function BackgroundStu() {
    ...
    const gradient = useMemo(() => {
        ...
        const gradient = ctx.createLinearGradient(0, 0, 0, 500); // Tạo gradient từ trên xuống dưới (theo trục y của canvas)
        gradient.addColorStop(0, 'rgba(75, 192, 192, 0.2)'); // Màu bắt đầu của gradient
        gradient.addColorStop(1, 'rgba(75, 192, 192, 0)'); // Màu kết thúc của gradient
        return gradient;
    }, []);
    // Từ MSSV, môn, lớp, tớ hy vọng cậu tìm object sinh viên đó cho tớ
                ...
                <div className = "Study-containt">
                    <ul>
                        ...
                        <li key = {index}>
                            ...
                            <div> <strong>Nhận xét :</strong> {item.comment}</div>
                            <div> <strong>Điểm : </strong>{item.rating +'/5'}</div>
                        </li>
                        )}
                    </ul>
                </div>
